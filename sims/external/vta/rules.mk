...
BASE_VFLAGS := -Wno-fatal --threads 2 -j `nproc` -O3 --compiler clang -MAKEFLAGS "OPT=-march=native" --Mdir $(VERILATOR_MDIR) --cc --build --exe
...
$(d)vta_rtl_sim_1x16_no_trace: $(d)vta_sim_rtl.sv $(d)vta_rtl_1x16.sv $(d)m_axil_adapter.sv $(d)s_axi_adapter.sv $(d)verilator_adapter.cc $(d)verilator_main.cc $(abspath $(lib_simbricks))
	verilator --top-module vta_sim -CFLAGS "$(CFLAGS)" -y $(VERILATOR_XILINX) $(VFLAGS) -o $(abspath $@) $^
